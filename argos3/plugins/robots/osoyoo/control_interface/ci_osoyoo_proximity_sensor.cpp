...
#include "ci_osoyoo_proximity_sensor.h"
...
namespace argos 
{
   ...
   CCI_OsoyooProximitySensor::CCI_OsoyooProximitySensor() :
      m_tReadings(7)
   {
   ...
   const CCI_OsoyooProximitySensor::TReadings& CCI_OsoyooProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_OsoyooProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_OsoyooProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_OsoyooProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_OsoyooProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
