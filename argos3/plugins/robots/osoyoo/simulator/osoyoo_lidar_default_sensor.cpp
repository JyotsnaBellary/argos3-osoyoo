/**
 * @file <argos3/plugins/robots/osoyoo/simulator/osoyoo_lidar_default_sensor.cpp>
 *
...
#include "osoyoo_lidar_default_sensor.h"
...
namespace argos {
   ...
   // static UInt8 OSOYOO_POWERON_LASERON   = 3;
   // const Real OSOYOO_BASE_ELEVATION = 0.045;
   // const Real OSOYOO_BASE_RADIUS    = 0.338;
   // const Real OSOYOO_BASE_HEIGHT    = 0.351f;
   // const Real OSOYOO_BASE_TOP       = OSOYOO_BASE_ELEVATION + OSOYOO_BASE_HEIGHT;
   // const Real OSOYOO_LIDAR_CENTER_ELEVATION   = -0.01;  // Lidar hight is considered in the robot height
   ...
   // const CRange<Real> OSOYOO_LIDAR_SENSORS_RING_RANGE(0.15f, 12.00f);
   ...
   COsoyooLIDARDefaultSensor::COsoyooLIDARDefaultSensor() :
      m_pnReadings(NULL),
      m_unNumReadings(1800),
      m_unPowerLaserState(OSOYOO_POWERON_LASERON),
   ...
   void COsoyooLIDARDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the Osoyoo LIDAR default sensor", ex);
      }
   }
   ...
   void COsoyooLIDARDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_OsoyooLIDARSensor::Init(t_tree);
         /* How many readings? */
   ...
   void COsoyooLIDARDefaultSensor::Update() {
      /* Nothing to do if sensor is deactivated */
      if(m_unPowerLaserState != OSOYOO_POWERON_LASERON)
         return;
   ...
   void COsoyooLIDARDefaultSensor::Reset() {
   ...
   void COsoyooLIDARDefaultSensor::Destroy() {
      delete[] m_pnReadings;
   }
   ...
   long COsoyooLIDARDefaultSensor::GetReading(UInt32 un_idx) const {
   ...
   size_t COsoyooLIDARDefaultSensor::GetNumReadings() const {
   ...
   void COsoyooLIDARDefaultSensor::PowerOn() {
      m_unPowerLaserState = m_unPowerLaserState | 0x1;
      m_pcProximityEntity->SetEnabled(m_unPowerLaserState == OSOYOO_POWERON_LASERON);
   }

   /****************************************/
   /****************************************/

   void COsoyooLIDARDefaultSensor::PowerOff() {
   ...
   void COsoyooLIDARDefaultSensor::LaserOn() {
   ...
   void COsoyooLIDARDefaultSensor::LaserOff() {
   ...
   REGISTER_SENSOR(COsoyooLIDARDefaultSensor,
                   "osoyoo_lidar", "default",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The Osoyoo LIDAR sensor.",
                   "This sensor accesses the Osoyoo LIDAR sensor. The sensors return the\n"
                   "distance to nearby objects. In controllers, you must include the\n"
                   "ci_osoyoo_lidar_sensor.h header.\n\n"
