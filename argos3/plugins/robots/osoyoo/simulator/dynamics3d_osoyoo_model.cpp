/**
 * @file <osoyoo/simulator/dynamics3d_osoyoo_model.cpp>
 *
 * @author Michael Allwright - <allsey87@gmail.com>
 */

#include "dynamics3d_osoyoo_model.h"
...
namespace argos {
   ...
   CDynamics3DOsoyooModel::CDynamics3DOsoyooModel(CDynamics3DEngine& c_engine,
                                                COsoyooEntity& c_osoyoo) :
      /* technically, the CDynamics3DMultiBodyObjectModel should be initialized with 7 children
         links, however, setting it to 7 makes the osoyoo less stable for reasons. */
      CDynamics3DMultiBodyObjectModel(c_engine, c_osoyoo, 3, false),
      m_cWheeledEntity(c_osoyoo.GetWheeledEntity()) {
      ...
      const SAnchor& sOriginAnchor = c_osoyoo.GetEmbodiedEntity().GetOriginAnchor();
      ...
      SAnchor* psBodyAnchor = &c_osoyoo.GetEmbodiedEntity().AddAnchor("body", {0.0, 0.0, 0.00125});
      /* create the bodies */
   ...
   void CDynamics3DOsoyooModel::Reset() {
   ...
   void CDynamics3DOsoyooModel::CalculateBoundingBox() {
   ...
   void CDynamics3DOsoyooModel::UpdateEntityStatus() {
   ...
   void CDynamics3DOsoyooModel::UpdateFromEntityStatus() {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DOsoyooModel::AddToWorld(btMultiBodyDynamicsWorld& c_world) {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DOsoyooModel::RemoveFromWorld(btMultiBodyDynamicsWorld& c_world) {
   ...
   REGISTER_STANDARD_DYNAMICS3D_OPERATIONS_ON_ENTITY(COsoyooEntity, CDynamics3DOsoyooModel);

   /****************************************/
   /****************************************/

   const btVector3    CDynamics3DOsoyooModel::m_cBodyHalfExtents(0.0362, 0.0236, 0.0362);
   const btScalar     CDynamics3DOsoyooModel::m_fBodyMass(0.242);
   const btTransform  CDynamics3DOsoyooModel::m_cBodyOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,0.00125,0.0));
   const btTransform  CDynamics3DOsoyooModel::m_cBodyGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0, -0.0236, 0.0));
   const btVector3    CDynamics3DOsoyooModel::m_cWheelHalfExtents(0.02125,0.0015,0.02125);
   const btScalar     CDynamics3DOsoyooModel::m_fWheelMass(0.006);
   const btTransform  CDynamics3DOsoyooModel::m_cWheelGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,-0.0015,0.0));
   const btTransform  CDynamics3DOsoyooModel::m_cLeftWheelOffset(btQuaternion(btVector3(-1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, -0.0255));
   const btTransform  CDynamics3DOsoyooModel::m_cRightWheelOffset(btQuaternion(btVector3(1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, 0.0255));
   const btVector3    CDynamics3DOsoyooModel::m_cBodyToRightWheelJointOffset(0.0, -0.0036, 0.0255);
   const btVector3    CDynamics3DOsoyooModel::m_cRightWheelToBodyJointOffset(0.0, 0.0015, 0.0);
   const btQuaternion CDynamics3DOsoyooModel::m_cBodyToRightWheelJointRotation(btVector3(-1,0,0), SIMD_HALF_PI);
   const btVector3    CDynamics3DOsoyooModel::m_cBodyToLeftWheelJointOffset(0.0, -0.0036, -0.0255);
   const btVector3    CDynamics3DOsoyooModel::m_cLeftWheelToBodyJointOffset(0.0, 0.0015, -0.0);
   const btQuaternion CDynamics3DOsoyooModel::m_cBodyToLeftWheelJointRotation(btVector3(1,0,0), SIMD_HALF_PI);
   /* TODO calibrate these values */
   const btScalar     CDynamics3DOsoyooModel::m_fWheelMotorMaxImpulse(0.15);
   const btScalar     CDynamics3DOsoyooModel::m_fWheelFriction(5.0);
